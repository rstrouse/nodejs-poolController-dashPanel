        ...
        _buildControls: function () {
            ...
            $.getApiService('/config/options/valves', null, function (opts, status, xhr) {
                ...
                for (var i = 0; i < valves.length; i++) {
                    $('<div />').appendTo(el).pnlValveConfig({ valveTypes: opts.valveTypes, maxValves: opts.maxValves, circuits: opts.circuits })[0].dataBind(valves[i]);
                }
            });
        }
    });
...
(function ($) {
    $.widget('pic.pnlValveConfig', {
        ...
        _buildControls: function () {
            ...
            $('<input type="hidden" data-datatype="bool" />').attr('data-bind', 'isIntake').appendTo(line);
            $('<input type="hidden" data-datatype="bool" />').attr('data-bind', 'isReturn').appendTo(line);
            $('<div />').appendTo(line).inputField({ required: true, labelText: 'Name', binding: binding + 'name', inputAttrs: { maxlength: 16 }, labelAttrs: { style: { marginRight: '.25rem' } } });
            ...
            var btnSave = $('<div id="btnSaveValve" />').appendTo(btnPnl).actionButton({ text: 'Save Valve', icon: '<i class="fas fa-save" />' });
