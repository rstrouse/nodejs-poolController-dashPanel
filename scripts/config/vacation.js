(function ($) {
    $.widget('pic.pnlVacation', {
        ...
        _buildControls: function () {
            ...
            $('<input type="date" class="vacation-startDate" style="padding:0.2rem 0.4rem" disabled/>').appendTo(line);
            line = $('<div class="vacation-dates"></div>').appendTo(pnl);
            $('<label style="display:inline-block;width:7rem;font-weight:bold">End Date:</label>').appendTo(line);
            $('<input type="date" class="vacation-endDate" style="padding:0.2rem 0.4rem" disabled/>').appendTo(line);
            $('<div style="margin-top:0.4rem;font-size:0.85rem;font-style:italic;color:#c60"><i class="fas fa-exclamation-triangle"></i> Vacation mode must be enabled/disabled from the OCP or ICP panel due to controller bug.</div>').appendTo(pnl);
        },
