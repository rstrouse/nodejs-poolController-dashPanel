            ...
            el.on('click', 'div.picBodySetPoints', function (evt) {
                ...
                $.getApiService('/config/body/' + el.attr('data-id') + '/heatModes', null, function (data, status, xhr) {
                    ...
                    divPopover.on('initPopover', function (evt) {
                        ...
                        $('<div></div>').appendTo(evt.contents()).valueSpinner({ canEdit: true, labelText: 'Set Point', val: settings.setPoint, min: units === 'C' ? 5 : 65, max: units === 'C' ? 41 : 104, step: 1, bind:'heatSetpoint', units: '<span>&deg;</span><span class="picTempUnits">' + units + '</span>', labelAttrs: { style: { marginRight: '.25rem' } } });
